...
class ResponseCrew:
    ...
    @agent
    def response_correction_agent(self) -> Agent:
        return Agent(
            llm=llama_llm,
            max_iter=15,
            config=self.agents_config["response_correction_agent"],
            verbose=True,
        )
    
    @agent
    def response_agent_2(self) -> Agent:
        return Agent(
            llm=llama_llm,
            max_iter=15,
            config=self.agents_config["response_agent_2"],
            verbose=True,
        )
